...
class Pose3D_realDataWriterListener : public dds::pub::NoOpDataWriterListener<dds::physics::Pose3D_real> {
  public:
    // Notifications about data
    void on_offered_deadline_missed(
        dds::pub::DataWriter<dds::physics::Pose3D_real>& writer,
...
void run_participant_application(unsigned int domain_id)
{
    ...
    dds::core::status::StatusMask Pose3D_real_status_mask;
    Pose3D_real_status_mask |= dds::core::status::StatusMask::offered_deadline_missed()
    ...
    auto Pose3D_real_listener = std::make_shared<Pose3D_realDataWriterListener>();
    dds::pub::DataWriter<dds::physics::Pose3D_real> Pose3D_real_writer(
        publisher,
        Pose3D_real_topic,
    ...
    dds::physics::Pose3D_real Pose3D_real_data;
    ...
    while (1) {
        ...
        Pose3D_real_data.orientation().x(randFloatInRange(0.0f, 6.28f));
        Pose3D_real_data.orientation().y(randFloatInRange(0.0f, 6.28f));
        Pose3D_real_data.orientation().z(randFloatInRange(0.0f, 6.28f));
        Pose3D_real_data.orientation().w(randFloatInRange(0.0f, 0.123f));
        Pose3D_real_data.position().x(randFloatInRange(-100.0f, 100.0f));
        Pose3D_real_data.position().y(randFloatInRange(-100.0f, 100.0f));
        Pose3D_real_data.position().z(randFloatInRange(0.10f, 0.80f));

        std::cout << "Writing dds::physics::Pose3D_real, count " << samples_written << std::endl;
        Pose3D_real_writer.write(Pose3D_real_data);
        samples_written++;
