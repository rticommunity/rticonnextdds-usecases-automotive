...
#include "{{Loop:Line:Topic:True:HeaderFileNameBase}}Application.h"
...
template <typename T>
void take_and_print(typename T::DataReader* reader, DDS_Long *total_samples)
{
...
{{Repl:DataTypePathAndNameUnderscored}}ReaderListener::on_data_available(DDSDataReader *reader)
{
    ...
    take_and_print<{{Repl:DataTypePathAndNameUnderscored}}>(hw_reader, &total_samples);
}
...
{{Loop:Start:Topic:DWListener:all}}
    *{{Repl:DataTypeName}}_sample = {{Repl:DataTypePathAndNameUnderscored}}TypeSupport::create_data();
    ...
    {
        printf("failed {{Repl:DataTypePathAndNameUnderscored}}_create\n");
        goto done;
    }
{{Loop:End:Topic:DWListener:all}}
{{Loop:Start:Topic:DRListener:all}}
...
{{Loop:End:Topic:DRListener:all}}
    ...
    {
{{Loop:Start:Topic:DWListener:all}}
        retcode = {{Repl:DataTypeName}}_hw_writer->write(*sample, DDS_HANDLE_NIL);
        if (retcode != DDS_RETCODE_OK)
        {
